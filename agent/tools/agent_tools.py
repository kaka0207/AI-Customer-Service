...
def _gaode_get(path: str, params: dict) -> dict:
    ...
    if not gaode_key or gaode_key.lower() in _GAODE_PLACEHOLDER_KEYS:
...
def rag_summarize(query: str) -> str:
    try:
        ...
        logger.error(f"[rag_summarize]知识库检索失败 err={str(e)}", exc_info=True)
        return "知识库暂未初始化成功，请检查 DASHSCOPE_API_KEY、向量库目录以及依赖环境后重试。"
